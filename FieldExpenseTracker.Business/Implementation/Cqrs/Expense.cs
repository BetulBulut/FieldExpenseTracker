...
public record GetAllExpensesByParameterQuery(int? EmployeeId, string? ExpenseNumber, string? Description, string? ExpenseCategory, int? Amount,string? ResponsedByUserName) : IRequest<ApiResponse<List<ExpenseResponse>>>;
...
public record CreateMultipleExpenseCommand(CreateMultipleExpenseRequest Expenses) : IRequest<ApiResponse<CreateMultipleExpenseResponse>>;
